...
# Función para extraer los parametros de SMPL-X y de la cámara
def extract_smplx_params(humans, K):
    smplx_params_list = []
    for human in humans:
        smplx_params = {
            ...
            "camera": {
                "focalLength": [K[0,0,0].item(), K[0,1,1].item()],
                "principalPoint": [K[0,0,2].item(), K[0,1,2].item()],
                "imageSize": [K.shape[1], K.shape[2]]  # Corregido aquí
            }
        }
...
if __name__ == "__main__":
        ...
        for i, img_path in enumerate(tqdm(l_img_path)):
            ...
            # Extract SMPL-X and camera parameters
            if args.save_json:
                smplx_params = extract_smplx_params(humans, K)

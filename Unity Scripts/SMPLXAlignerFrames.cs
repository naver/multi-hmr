...
public class SMPLXAlignerFrames : MonoBehaviour
{
    ...
    public Button nextFrameButton; // Botón para avanzar al siguiente frame
    public VideoPlayer backgroundVideoPlayer; // Componente VideoPlayer para el video de fondo
    public RawImage videoDisplay; // RawImage en el Canvas para mostrar el video
    ...
    private SMPLX smplxComp;

    void Start()
    {
        frames = JSONReaderFrames.ReadJSONFile(jsonFilePath);
        if (frames != null)
        {
            smplxInstance = Instantiate(smplxPrefab); // Instanciar solo si no existe
            smplxComp = smplxInstance.GetComponent<SMPLX>();

            SetupCamera();
            AlignSMPLX(currentFrame);
            SetupBackgroundImage();
            smplxInstance.transform.rotation *= Quaternion.Euler(0, 180, 0);
        }
    ...
    {
        float fx = frames[0].camera_intrinsics[0][0];
        float fy = frames[0].camera_intrinsics[1][1];
        float cx = frames[0].camera_intrinsics[0][2];
        float cy = frames[0].camera_intrinsics[1][2];

        float fovY = 2 * Mathf.Atan(frames[0].resized_height / (2 * fy)) * Mathf.Rad2Deg;
        alignmentCamera.fieldOfView = fovY;

        float aspect = (float)frames[0].resized_width / frames[0].resized_height;
        alignmentCamera.aspect = aspect;
    }

    private void AlignSMPLX(int frameIndex)
    {
        ...
        foreach (Human human in frameData.humans)
        {
            ...
            Debug.Log($"Chequeo de frames: {human.rotation_vector[0][0]}");
    ...
    private void OnNextFrameButtonClick()
    {
        ...
        {
            currentFrame = 0; // Volver al primer frame si se llega al final
        }

        // Cambiar al siguiente frame del video
        if (backgroundVideoPlayer != null && backgroundVideoPlayer.isPrepared)
        {
            long totalFrames = (long)backgroundVideoPlayer.frameCount;
            backgroundVideoPlayer.frame = currentFrame % totalFrames;
        }

        // Limpiar la escena antes de aplicar el siguiente frame
        ClearScene();
    ...
    {
        if (backgroundVideoPlayer != null && videoDisplay != null)
        {
            ...
            backgroundVideoPlayer.url = "C:/Users/sebao/Downloads/input(2).mp4"; // Cambia esto por la ruta real de tu video
            backgroundVideoPlayer.playOnAwake = false;
            backgroundVideoPlayer.isLooping = true;
            backgroundVideoPlayer.frame = 0;
            backgroundVideoPlayer.Prepare(); // Preparar el video para la reproducción
        }
    }
    ...
    private void ClearScene()
    {
        ...
        jointObjects.Clear();
        if (joint3dParentGO != null)
        {
            Destroy(joint3dParentGO);
        }
    }
}
